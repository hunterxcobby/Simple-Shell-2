...
int main(int ac, char **av)
{
	...
	while (1)
	{
		...
		{
			prompt_bytes_written = write(1, prompt, custom_string_length(prompt));
		...
		/* if command is exit */
		if (custom_string_compare(line, "exit") == 0)
		{
			free(line);
			exit(0);
		}
		...
		/* if command is env */
		if (custom_string_compare(line, "env") == 0)
		{
			int i = 0;
			...
			for (; environ[i]; i++)
			{
				...
				write(1, environ[i], custom_string_length(environ[i]));
				write(1, "\n", 1);
			}
			free(line);
			continue;
		...
		while (command_token != NULL)
		{
			...
			command[i] = custom_string_duplicate(command_token);
			if (command[i] == NULL)
				continue;
		...
		if (path == NULL)
		{
			...
			path_env_var = custom_string_duplicate(PATH);
			...
			while (directories_token != NULL)
			{
				...
				directories[directories_i] = custom_string_duplicate(directories_token);
				if (PATH == NULL)
					continue;
			...
			while (directories[searching_i] != NULL)
			{
				...
				while (entry != NULL)
				{
					if (custom_string_compare(command[0], entry->d_name) == 0)
					{
						found = 1;
						break;
					}
		...
		else if (child_pid == 0)
		{
			...
			{
				size = custom_string_length(directories[searching_i]) +
				custom_string_length(line) + 2;
				...
				custom_string_copy(concat_path, directories[searching_i]);
				custom_concatenate(concat_path, "/");
				custom_concatenate(concat_path, command[0]/*line*/);
